...
export default function ClickableCover({ src, isSensitive, className }: { src: string; isSensitive?: boolean; className?: string }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <img src={src} alt="" className={className} style={{ filter: revealed ? "none" : "blur(12px)", transition: "filter 0.2s", cursor: isSensitive ? "pointer" : undefined }} onClick={handleReveal} onError={() => setImgError(true)} />
