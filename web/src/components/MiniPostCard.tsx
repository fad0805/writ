    ...
    >
      ...
      <div className="mini-post-content">
        ...
        {(post as any).media_attachments?.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: (post as any).media_attachments.length <= 2 ? "1fr" : "1fr 1fr", gridAutoRows: "120px", gap: 2, marginTop: 6, borderRadius: 6, overflow: "hidden" }}>
            {(post as any).media_attachments.slice(0, 4).map((m: any, i: number) => (
              m.type === "video" ? (
                ...
                <img key={i} src={m.url} alt={m.alt || ""} style={{ width: "100%", height: "100%", objectFit: "contain", background: "#000", borderRadius: 4 }} />
              )
            ))}
          </div>
        )}
