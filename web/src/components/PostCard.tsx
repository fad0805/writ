             ...
             {(post as any).media_attachments.slice(0, 16).map((m: any, i: number) => {
              ...
              return m.type === "video" ? (
                <div key={i} style={{ position: "relative", lineHeight: 0 }}>
                  ...
                  {revealed && <button onClick={(e) => { e.stopPropagation(); setRevealedSensitive((prev) => { const n = new Set(prev); n.delete(i); return n; }); }} style={{ position: "absolute", top: 4, right: 4, zIndex: 2, background: "rgba(0,0,0,0.6)", border: "none", borderRadius: 4, color: "#fff", fontSize: 12, padding: "3px 10px", cursor: "pointer" }}>민감</button>}
