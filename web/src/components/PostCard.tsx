      ...
      <div style={{ position: "relative", marginTop: 8, overflow: "hidden", borderRadius: 8 }}>
        {single ? (
          <div className="post-media-single">
            {media.slice(0, 1).map((m: any, i: number) => {
              ...
              return m.type === "video" ? (
                ...
                <div key={i} style={{ position: "relative", lineHeight: 0, overflow: "hidden", background: "#000", borderRadius: 6 }}>
                  {blurred && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", borderRadius: 6, zIndex: 1 }} />}
                  <img src={m.url} alt={m.alt || ""} style={{ width: "100%", maxHeight: 400, borderRadius: 6, objectFit: "contain", background: "#000", cursor: blurred ? "default" : "pointer", filter: blurred ? "blur(20px)" : "none" }} onClick={(e) => { if (!blurred) { e.stopPropagation(); setViewerIndex(i); } }} />
                </div>
              );
            })}
          </div>
          ...
          <div className="post-media-grid" style={{ display: "grid", gridTemplateColumns: `repeat(${n <= 2 ? n : n <= 4 ? 2 : 3}, 1fr)`, gridAutoRows: "200px", gap: 2 }}>
            {media.slice(0, 16).map((m: any, i: number) => {
              ...
              return m.type === "video" ? (
                ...
                <div key={i} style={{ position: "relative", lineHeight: 0, overflow: "hidden", background: "#000", borderRadius: 6 }}>
                  {blurred && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", borderRadius: 6, zIndex: 1 }} />}
                  <img src={m.url} alt={m.alt || ""} style={{ width: "100%", height: "100%", borderRadius: 6, objectFit: "cover", background: "#000", cursor: blurred ? "default" : "pointer", filter: blurred ? "blur(20px)" : "none" }} onClick={(e) => { if (!blurred) { e.stopPropagation(); setViewerIndex(i); } }} />
                </div>
              );
            })}
          </div>
    ...
    <>
      ...
      {!readonly && showRewrite && post.reply_context && (
        <ReplyModal post={{
          ...
          reply_context: null,
          media_attachments: [],
        } as any} initialContent={rewriteContent ?? undefined} initialSummary={rewriteSummary} initialMedia={rewriteMedia} onClose={() => { setShowRewrite(false); setRewriteContent(null); setRewriteSummary(""); setRewriteMedia([]); }} onDone={(newPost) => {
      ...
      {!readonly && showRewrite && !post.reply_context && (
        <div className="reply-modal-backdrop active" onClick={() => { setShowRewrite(false); setRewriteContent(null); setRewriteSummary(""); setRewriteMedia([]); }}>
          <div className="reply-modal modal-form" onClick={(e) => e.stopPropagation()}>
            <button className="reply-modal-close" onClick={() => { setShowRewrite(false); setRewriteContent(null); setRewriteSummary(""); setRewriteMedia([]); }}>×</button>
            <h3>지우고 다시 쓰기</h3>
            <PostForm onDone={(newPost) => {
              ...
              setRewriteSummary("");
              setRewriteMedia([]);
            }} initialContent={rewriteContent ?? undefined} initialSummary={rewriteSummary} initialVisibility={post.visibility} initialMedia={rewriteMedia} />
          </div>
        </div>
      )}
      {reactionTooltip && (
