      ...
      <div className="feed">
        {loading ? (
          ...
          <InfiniteScroll hasMore={hasMore} loadingMore={loadingMore} loadMore={loadMore}>
            {posts.filter((p) => !deletedIds.current.has(p.id)).map((p, i) => <div key={p.id} ref={(el) => { cardRefs.current[i] = el; }}><PostCard post={p} onDelete={() => { deletedIds.current.add(p.id); setPosts((prev) => prev.filter((x) => x.id !== p.id)); }} onUpdate={(updated) => { if (updated) { console.log("[EDIT] updating post", p.id, "new content:", updated.content?.slice(0, 50)); setPosts((prev) => { const next = prev.map((x) => x.id === p.id ? updated : x); console.log("[EDIT] posts after update:", next.find(x => x.id === p.id)?.content?.slice(0, 50)); return next; }); } else { api.getPost(p.id).then((u) => setPosts((prev) => prev.map((x) => x.id === p.id ? u : x))).catch(() => {}); } }} onReply={(newPost) => { if (newPost) { setPosts((prev) => { if (prev.some((x) => x.id === newPost.id)) return prev; return [newPost, ...prev]; }); } }} selected={i === selectedIdx} /></div>)}
          </InfiniteScroll>
        )}
      </div>
