...
export default function NovelByNumberPage() {
    ...
    fetch(`/api/by-series-number/${params.username}/${params.number}`, { credentials: "include" })
      ...
      .then((d) => {
        ...
        api.getNovel(d.id).then((nd) => {
          ...
          setAuthor(nd.author); setIsMine(nd.is_mine); setIsFollowing(nd.is_following); setLoading(false);
          if (user) {
            fetch("/api/mutes/series", { credentials: "include" })
              .then((r) => r.json())
              .then((m: MutesResp) => setIsSeriesMuted(m.mutes?.some((mt) => mt.novel_id === d.id) || false))
              .catch(() => {});
            setIsSeriesPinned(user.pinned_series?.includes(d.id) || false);
          }
  ...
  }, [params.username, params.number, user]);
            ...
            <div className="series-header-top">
              ...
              <div className="series-header-btns">
                ...
                <div className="series-more-wrap">
                  ...
                  {showShareMenu && (
                    <div className="series-more-menu" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(window.location.origin + `/series/by-number/${params.username}/${params.number}`); setShowShareMenu(false); }}><Icon name="link" /> 링크 복사</button>
                    </div>
                  )}
                </div>
                <div className="series-more-wrap" onClick={() => setShowMoreMenu(!showMoreMenu)}>
                  ...
                  {showMoreMenu && (
                    <div className="series-more-menu" onClick={(e) => e.stopPropagation()}>
                      {novel.visibility !== "private" && <button onClick={() => { navigator.clipboard.writeText(window.location.origin + `/series/by-number/${params.username}/${params.number}`); setShowMoreMenu(false); }}><Icon name="link" /> 링크 복사</button>}
                      {user && !isMine && (
